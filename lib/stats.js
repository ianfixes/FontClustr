...
(function(root) {
	...
	stats.init.prototype = {
		...
		// Finds a quantile with the given ratio
		//
		// Returns that quantile.
		quantile: function(ratio) {
			// Sort the numbers
			var nums = this.clone().sort();
                        var arr = nums.arr;

                        // 0 and 1 are bounds; clamp to them
                        if (ratio <= 0.0) return arr[0];
                        if (ratio >= 1.0) return arr[arr.length - 1];

                        // Determine which index represents the quantile,
                        // averaging if it falls on a boundary
                        var idx = (ratio * arr.length) - 1;
                        if (idx === Math.floor(idx)) {
                              return 0.5 * (arr[idx] + arr[idx + 1]);
                        } else {
                              return arr[Math.ceil(idx)];
                        }
		},
